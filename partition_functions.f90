...
module partition_functions
    ...
    contains
                ...
                associate(f => clusterset(iclust)%frequencies, q => lnq(iclust), &
                    ...
                    do ifreq = 1, size(f)
                        ...
                        if (x > 0.0_dp) then
                            ...
                            q = q - 0.5_dp*t_vib/temp - log(1.0_dp - exp(-t_vib/temp))
                        end if
                    end do
                end associate
            end do
        ...
        subroutine calculate_emf(emf, composition, amf, vol)
            ...
            case(1)
                emf = -1.0_dp/vol*global_data%ntot(1)*real(composition(1), dp)*amf
            case(2)
                emf = - 1.0_dp/vol*( &
                    global_data%ntot(1)*real(composition(1), dp)*amf_pure(1) + &
                    global_data%ntot(2)*real(composition(1), dp)*amf + &
                    global_data%ntot(1)*real(composition(2), dp)*amf + &
                    global_data%ntot(2)*real(composition(2), dp)*amf_pure(2) )
            end select
        end subroutine calculate_emf
        !=================================================================================
        ! Calculates the temperature derivative of the translational partition function.
        subroutine calculate_dlnqtrans(dlnq, temp)
        ...
        subroutine calculate_dlnqint(dlnq, temp, amf, vol)
            real(dp), dimension(:), intent(out) :: dlnq
            real(dp), intent(in)  :: temp
            real(dp), intent(in)  :: amf
            real(dp), intent(in)  :: vol
        ...
        ! function.
        subroutine calculate_ddlnqtrans(dlnq, temp)
            real(dp), dimension(:), intent(out) :: dlnq
            real(dp), intent(in)  :: temp
        ...
        ! function.
        subroutine calculate_ddlnqint(dlnq, temp, amf, vol)
            real(dp), dimension(:), intent(out) :: dlnq
            real(dp), intent(in)  :: temp
            real(dp), intent(in)  :: amf
            real(dp), intent(in)  :: vol
